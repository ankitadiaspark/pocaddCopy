...
@Component({
  selector: "app-widget-gridster",
  templateUrl: "./widget-gridster.component.html",
  styleUrls: ["./widget-gridster.component.scss"],
  encapsulation: ViewEncapsulation.None
})
export class WidgetGridsterComponent implements OnInit {
  chartReady: boolean;
  $: any;
  ...
  constructor(private widgetService: WidgetService) {
    widgetService.getWidgetsGrid().subscribe(
      widgetResponse => {
        ...
        widgetService.getWidgetOrder().subscribe(orderResponse => {
          ...
          widgetService.getWidgets().subscribe(widgetSectionResponse => {
            var sectionResponse = widgetSectionResponse;
            _.each(this.order, item => {
              var widgetSection = _.where(sectionResponse, { id: item })[0];
              console.log(widgetSection, "widgetSection");
              console.log(widgetSection.id, "widgetidSection");
              this.sections.push(widgetSection);
            });

            console.log("section", this.sections);
            console.log("widgetorder", this.widgets);
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    this.widgetService.getWidgetsGrid().subscribe(
      widgetResponse => {
        var widgets = widgetResponse;
        this.widgetService.getWidgetOrder().subscribe(orderResponse => {
          this.order = orderResponse.ids;

          this.widgetService.getWidgets().subscribe(widgetSectionResponse => {
            var sectionResponse = widgetSectionResponse;
